...
import it.polimi.tiw.civitas.service.LawWorkflowException;
...
@WebServlet("/law/repeal")
public class RepealLawServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            lawWorkflowService.repealLaw(lawId, loggedUser.getId());
            response.sendRedirect(request.getContextPath() + "/law?id=" + lawId);

        } catch (LawWorkflowException e) {
            String encodedError = URLEncoder.encode(e.getMessage(), StandardCharsets.UTF_8);
            response.sendRedirect(request.getContextPath() + "/law?id=" + lawId + "&workflowError=" + encodedError);
